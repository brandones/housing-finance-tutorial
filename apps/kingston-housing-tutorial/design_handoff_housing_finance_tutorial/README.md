...
- **NOI insufficient warning:** when `noi < debtService * 1.2` (1.2× DSCR), show a small italic note in terracotta below the NOI row: `NOI is less than the loan requirement of $X,XXX`. (See original Svelte reference for exact behavior.)
- **Cash flow color:** if cash flow is negative, render the value in terracotta `#b87351`; otherwise default ink `#2b2724`. (Optional: tint the result row's top-border to match.)
...
| `--color-out-soft` | `#efddd1` | (reserved for warnings/states) |
...
- `index.html` — the brainstorm canvas containing all six visualization variations and five input-affordance variations. **Variation A (signed math)** in `variations.jsx` and **Variation IA (hairline chip)** in `input-affordances.jsx` are the chosen direction.
